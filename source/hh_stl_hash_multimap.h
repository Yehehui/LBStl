#ifndef __HH_STL_HASH_MULTIMAP_H_
#define __HH_STL_HASH_MULTIMAP_H_
...
template <class Key, class T, class HashFun, class EqualFun, class Alloc>
inline bool operator== (const hash_multimap<Key, T, HashFun, EqualFun, Alloc> &x, const hash_multimap<Key, T, HashFun, EqualFun, Alloc> &y) {
    return x.ht == y.ht;
}
...
public:
    ...
    hash_multimap(): ht(50, hash_fun(), key_eq()) {}
    explicit hash_multimap(const size_type n): ht(n, hash_fun(), key_eq()) {}
    hash_multimap(const size_type n, const hasher &h): ht(n, h, key_eq()) {}
    hash_multimap(const size_type n, const hasher &h, const key_equal &k): ht(n, h, k) {}

    template <class InputIterator>
    hash_multimap(InputIterator first, InputIterator last): ht(50, hash_fun(), key_eq()) {
        ht.insert_equal(first, last);
    }
    template <class InputIterator>
    hash_multimap(InputIterator first, InputIterator last, const size_type n): ht(n, hash_fun(), key_eq()) {
        ht.insert_equal(first, last);
    }
    template <class InputIterator>
    hash_multimap(InputIterator first, InputIterator last, const size_type n, const hasher &h): ht(n, h, key_eq()) {
        ht.insert_equal(first, last);
    }
    template <class InputIterator>
    hash_multimap(InputIterator first, InputIterator last, const size_type n, const hasher &h, const key_equal &k): ht(n, h, k) {
        ht.insert_equal(first, last);
    }
    ...
    void swap(hash_multimap<Key, T, HashFun, EqualFun, Alloc> &x) {
        ht.swap(x.ht);
    }
    iterator begin() {
        return ht.begin();
...
template <class Key, class T, class HashFun, class EqualFun, class Alloc>
inline void swap(hash_multimap<Key, T, HashFun, EqualFun, Alloc> &x, hash_multimap<Key, T, HashFun, EqualFun, Alloc> &y) {
    x.swap(y);
}
